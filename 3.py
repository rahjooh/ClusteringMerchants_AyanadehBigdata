import glob , pandas as pd ,time,sklearn.cluster as cluster,numpy as np
...
def plot_clusters(pddf2,data, algorithm, args, kwds , st, Recency = False ):
    ...
    if Recency:
        ...
        data = data[['M','F']]
    column_name = st + '_' + str(algorithm)[str(algorithm).rfind('.') + 1:str(algorithm).rfind("'")] + str(args) + str(
        kwds)
    print()
    print(time.ctime())
    print(column_name)
...
def preprocessing (path , status , normalization =False ,DimReduc=False , Recency = False):
    ...
    for m in l1 :
        ...
        print('++++++++++++++',m,'++++++++++++++++')
        pddf_all.to_csv('tempresult_'+str(m) + st + '.csv')
        # if result.empty:
        #     result = pddf_all[:]
        # else:
...
state = ['Quintuple','Fixed3','Fixed6','Fixed9','Quirtile','real']
for st in state:
    preprocessing('Merchant_tagged_all.xlsx',status=st)
    print('===============================================================')
    #92pddf1.to_csv('result_'+st+'.csv')
