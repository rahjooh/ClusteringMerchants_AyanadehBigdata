...
def start_session():
    # delete_session(0)
    data = {'kind': 'pyspark'}
    response = requests.post("http://10.100.136.40:8070" + '/sessions', data=json.dumps(data), headers={'Content-Type': 'application/json; charset=utf-8'})
...
def get_all_sessions():
    response = requests.get("http://10.100.136.40:8070" + '/sessions', headers={'Content-Type': 'application/json; charset=utf-8'})
    return response.json()


def delete_session(session_id):
    response = requests.delete("http://10.100.136.40:8070"+ '/sessions/' + str(session_id), headers={'Content-Type': 'application/json; charset=utf-8'})
    return response.json()


def get_session_state(session_id):
    response = requests.get("http://10.100.136.40:8070" + "/sessions/" + str(session_id), headers={'Content-Type': 'application/json; charset=utf-8'})
...
def get_session(Spark_code):
    ...
    response = requests.post("http://10.100.136.40:8070" + "/sessions/" + str(session_id) + "/statements",
                             data=json.dumps(Spark_code),
                             headers={'Content-Type': 'application/json; charset=utf-8'}).json()
    while response["state"] != 'available':
        time.sleep(0.1)
        response = requests.get("http://10.100.136.40:8070" + "/sessions/" + str(session_id) + "/statements/" + str(response["id"]),
